...
fn create_diagram(segments: &Vec<&Segment>) -> Vec<i64> {
    let width = segments.iter().map(|seg| cmp::max(seg.start.x, seg.end.x)).max().unwrap() + 1;
    let height = segments.iter().map(|seg| cmp::max(seg.start.y, seg.end.y)).max().unwrap() + 1;
...
fn count_volatile_tiles(diagram: &Vec<i64>) -> usize {
    diagram.iter().filter(|cell| **cell > 1).collect::<Vec<&i64>>().len()
}
...
impl From<&str> for Segment {
    fn from(segment_string: &str) -> Self {
        let split: Vec<i64> = segment_string
            ...
            .collect();
        Segment {
            start: Point { x: split[0], y: split[1] },
            end: Point { x: split[2], y: split[3] }
        }
    }
}
