...
fn part_one(entries: &Vec<Entry>) {
    ...
    let values = entries.iter().fold(Vec::new(), |mut e, entry| {
        e.extend_from_slice(
            &entry
                .output_value
                .iter()
...
fn figure_out_wiring<'a>(examples: &'a Vec<&str>) -> [&'a str; 7] {
    ...
    assert!(
        three
            ...
            .filter(|letter| letter != &"")
            .filter(|letter| !one.contains(letter)
                && ![wiring_guide[0], wiring_guide[3]].contains(letter))
            .collect::<Vec<&str>>()
            .len()
            == 1
    );
    wiring_guide[6] = three
        ...
        .filter(|letter| letter != &"")
        .find(|letter| {
            !one.contains(letter) && ![wiring_guide[0], wiring_guide[3]].contains(letter)
        })
        .unwrap();
    // println!("result: {:?}", result);
