...
fn part_one(transparent_sheet: &Vec<Vec<usize>>, folding_instructions: &Vec<FoldingInstruction>) {
    ...
    println!(
        "{:?}",
        result.iter().fold(0 as usize, |count, row| count
            + row
                .iter()
                .filter(|cell| *cell > &0)
                .collect::<Vec<&usize>>()
...
fn split_transparent_sheet(
    transparent_sheet: &Vec<Vec<usize>>,
    folding_instruction: &FoldingInstruction,
    ...
    if folding_instruction.axis == "y" {
        ...
        (
            transparent_sheet
                .iter()
                .map(|row| row[0..value].to_vec())
                .collect(),
            transparent_sheet
                .iter()
                .map(|row| row[value + 1..].to_vec())
                .collect(),
        )
    }
}
