...
pub fn day_nine(input: &str) {
    let rows: Vec<Vec<ThreeDPoint>> = input
        .lines()
        .enumerate()
        .map(|(y ,line)| line.split("")
            .filter(|num| num != &"")
            .enumerate()
            .map(|(x, num)| ThreeDPoint { x: x as u16, y: y as u16, z: num.parse::<u16>().unwrap() })
...
fn part_one(lowpoints: &Vec<&ThreeDPoint>) {
    println!("pt 1");
    println!("{:?}", lowpoints.iter().map(|lowpoint| lowpoint.z + 1).sum::<u16>());
}
...
fn get_basinsize(lowpoint: &ThreeDPoint, data_rows: &Vec<Vec<ThreeDPoint>>) -> u32 {
    ...
    let mut queue = vec![ ThreeDPoint{ x: lowpoint.x, y: lowpoint.y, z: lowpoint.z } ];
    ...
    while queue.len() > 0 {
        ...
        queue.append(&mut get_neighbours(&data_rows, row_count, row_length, &current));
    }
...
fn get_lowpoints(data_rows: &Vec<Vec<ThreeDPoint>>) -> Vec<&ThreeDPoint> {
    ...
    for row in data_rows.iter() {
        for cell in row.iter() {
            if get_neighbours(
                &data_rows, row_count, row_length, &cell
            ).iter().all(|n| cell.z < n.z) {
                lowpoints.push(cell);
            }
        }
    }
